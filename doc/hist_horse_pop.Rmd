...
```{r plot horses, fig.cap = "Figure 1", fig.width=10, fig.height=3}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption="Table 1. The range of the number horses for each provinces ")
```

Below we zoom in and look at the province of `r max_horses$Province[1]`:

```{r plot province, fig.cap = "Figure 2", fig.width=5, fig.height=4}
